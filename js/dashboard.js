...
function refreshDashboard() {
  ...
  document.getElementById('st1').textContent=hist.filter(h=>h.resultado==='Negativo').length;
  document.getElementById('st2').textContent=hist.filter(h=>h.resultado==='Positivo').length;
  document.getElementById('st3').textContent=hist.filter(h=>h.resultado==='Pendiente').length;
  const wb=document.getElementById('recentWeeks');
  const rw=[...weeks].reverse().slice(0,8);
  wb.innerHTML=rw.length===0
    ?'<tr><td colspan="6" style="text-align:center;color:var(--gray-500);padding:24px">Sin registros</td></tr>'
    :rw.map(w=>{
      const wh=hist.filter(h=>h.planta===w.planta&&h.fecha===w.fecha);
      const pos=wh.filter(h=>h.resultado==='Positivo').length;
      const pend=wh.filter(h=>h.resultado==='Pendiente').length;
      const cls=pos>0?'badge-red':pend>0?'badge-yellow':'badge-green';
      const txt=pos>0?pos+' Positivo(s)':pend>0?'Pendiente':'Completo';
